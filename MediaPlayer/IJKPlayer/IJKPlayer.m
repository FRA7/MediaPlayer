...
@interface IJKPlayer()<UIGestureRecognizerDelegate>{
...
//工具蒙版
@property (nonatomic,strong)IJKButton * btnOperate;
@property (nonatomic,strong)IJKButton * btnLock;
@property (nonatomic,strong)IJKLoading * loading;

//标题栏
@property (nonatomic,strong)IJKButton * btnBack;
@property (nonatomic,strong)UILabel * lblTitle;

//工具蒙版
@property (nonatomic,strong)IJKButton *btnPlay;
@property (nonatomic,strong)IJKButton *btnFullScreen;
...
- (NSMutableArray *)thumbImages
{
    if (_thumbImages == nil) {
        ...
        for (int i = 1; i<=5; i++) {
            NSString *filename = [NSString stringWithFormat:@"thumb%d", i];
...
-(void)setupPlayerView{
    ...
    [_btnBack mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(self.cover.mas_left);
        make.top.equalTo(self.cover.mas_top).offset(10);
    ...
    //全屏按钮
    _btnFullScreen = [[IJKButton alloc]init];
    ...
    [_btnFullScreen mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(self.cover);
        make.bottom.mas_equalTo(self.cover);
    ...
    [_lblCurrentTime mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.cover).offset(10);
    ...
    [_progressView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.lblCurrentTime.mas_right).offset(5);
        make.right.equalTo(self.lblTotalTime.mas_left).offset(-5);
        make.centerY.equalTo(self.btnFullScreen);//progressView向下一个像素
    }];
    ...
    [_btnPlay mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.centerY.equalTo(self.cover);
        make.width.height.mas_equalTo(100);
    }];
    
    _thumbnailImageView = [[UIImageView alloc] init];
    [_cover addSubview:_thumbnailImageView];
    [_thumbnailImageView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.centerY.equalTo(self.cover);
        make.width.mas_equalTo(100);
        make.height.mas_equalTo(56.25);
    }];
    
    
    _btnLock = [[IJKButton alloc]init];
...
//    [_btnLock addTarget:self action:@selector(lock:) forControlEvents:(UIControlEventTouchUpInside)];
    ...
    [_btnLock mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self.btnPlay);
        make.left.equalTo(self.btnBack);
        make.width.height.mas_equalTo(40);
    }];
    
    //loading的位置用计算的...
    _loading = [[IJKLoading alloc]init];
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    if ([touch.view isKindOfClass:[IJKButton class]]){
        return NO;
    }
    return YES;
}
...
-(void)sliderValuechange:(UISlider *)sender{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        int imageIndex = sender.value * self.thumbImages.count;
        if (imageIndex < self.thumbImages.count) {
            self.thumbnailImageView.image = self.thumbImages[imageIndex];
        }
    });
...
- (void)fullScreen:(IJKButton *)sender {
